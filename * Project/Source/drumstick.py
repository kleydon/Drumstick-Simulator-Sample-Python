...
def calcHitVolume():
    ...
    print "LOWER_LIM:%0.2f    "\
        "hitMaxAbsAccel:%0.2f    "\
        "UPPER_LIM:%0.2f    "\
        "Volume:%0.2f" % \
        (LOWER_LIM, hitMaxAbsAccel, UPPPER_LIM, v)
    return v
...
# Set up serial comms
sio = serial.Serial(
    '/dev/tty.usbmodem753371',
    57600, timeout=1)  # USB is always really 12 Mbit/sec (Teensyduino)
sio.flush()
...
mixer.init()
audioFile = "./../Sounds/Steel-Drum-C4.wav"
...
while True:
    ...
    if len(line) not 8:
        ...
        if (calculatedChecksum not receivedChecksum):
            dummy = 0
            ...
            if state == STATE_WAITING_FOR_HIT:
                ...
                # Update number of consecutive ABSOLUTE slopes
                ...
                # If impact found...
                # if (consecutiveAbsSlopesAboveThresholdCount >
                ...
                if (consecutiveSlopesAboveThresholdCount >
                    ...
                    if hitDurationSampleCount >= MAX_HIT_DURATION_SAMPLE_COUNT:
                        ...
                        # print "Hit #%d (s/sec: %0.2f)" %
                        #         (hitCount, samplesPerSec)
                        # print "maxAccel: %0.2f  maxSlope: %0.2f vol: %0.2f" %
                        # (hitMaxAbsAccel, hitMaxAbsSlope, hitVolume)
